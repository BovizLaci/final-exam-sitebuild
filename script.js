...
function fillCard(card, book, index) {
    card.querySelector("p.no").innerText = index;
    card.querySelector("h2.title").innerText = book.title;
    card.querySelector("p.sub").innerText = book.sub;
    card.querySelector("p.text").innerText = book.text;
}

function addAndFillCards() {
	const cardsContainer = document.querySelector("main");
	const firstBookCard = document.querySelector("article.book");
    fillCard(firstBookCard, books[0], 1);
    for (let i = 1; i < books.length; i++) {
        const newBookCard = firstBookCard.cloneNode(true);
        fillCard(newBookCard, books[i], i+1);
        cardsContainer.appendChild(newBookCard);
    }
}
